...
Речь о том, чтобы **процесс JVM Tomcat** стартовал с **`-javaagent`**: тогда срабатывает `Premain-Class` из собранного JAR (`io.github.michael1297.agent.MixinDemoPremain`) и миксины могут менять байткод классов, которые позже загрузит Tomcat (в том числе классы из вашего WAR).
